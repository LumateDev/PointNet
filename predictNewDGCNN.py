"""
Скрипт для предсказания сегментации с использованием обученной DGCNN модели
Применение к LAS файлу и сравнение с эталоном
"""
...
# Импорт модели
from modelDGCNN import DGCNN_Segmentation

def predict_las_file_with_comparison(input_las_unlabeled, reference_las, model_path, output_las, num_classes=4, num_points=4096, batch_size=8):
    """
    Применение обученной DGCNN модели к неразмеченному облаку точек и сравнение с эталоном
    """
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    print(f"🖥️  Устройство: {device}")
    
    # Загрузка модели
    print(f"\n📥 Загрузка DGCNN модели из {model_path}...")
    try:
        model = DGCNN_Segmentation(num_classes=num_classes).to(device)
        checkpoint = torch.load(model_path, map_location=device)
        model.load_state_dict(checkpoint['model_state_dict'])
        model.eval()
        print("✅ DGCNN модель загружена")
    ...
    print("\n📦 Разбиение на блоки...")
    block_size = 50.0
    stride = 25.0  # С перекрытием для лучшего покрытия
    ...
    with torch.no_grad():
        for block_idx, (block_pts, indices) in enumerate(tqdm(zip(blocks, block_indices), total=len(blocks))):
            ...
            # Сохраняем предсказания (DGCNN предсказывает индексы 0-3, переводим в оригинальные классы если нужно)
            # Для простоты оставим как есть (0-3), можно будет маппить позже
            predictions[indices] = block_predictions
    
    # Вычисление точности по сравнению с эталоном
    print(f"\n📊 Вычисление точности по сравнению с эталоном...")
    
    # Приведение эталонных меток к numpy массиву
...
def main():
    ...
    REFERENCE_LAS = 'datasets/raw/NEONDSSampleLiDARPointCloud.las'              # Эталонный файл для сравнения
    OUTPUT_LAS = 'predicted_dgcnn_with_comparison.las'                          # Выходной файл с предсказаниями
    MODEL_PATH = 'checkpoints/best_dgcnn.pth'                                   # Путь к обученной DGCNN модели
    
    # Проверяем существование файлов
    if not os.path.exists(INPUT_LAS_UNLABELED):
        print(f"❌ Файл не найден: {INPUT_LAS_UNLABELED}")
        return
    if not os.path.exists(REFERENCE_LAS):
        print(f"❌ Файл не найден: {REFERENCE_LAS}")
        return
    ...
    try:
        predict_las_file_with_comparison(INPUT_LAS_UNLABELED, REFERENCE_LAS, MODEL_PATH, OUTPUT_LAS, num_classes=4)
    except Exception as e:
